...
import { cancelCabBooking, getCabBookingsByEmail } from "../lib/api";
...
export default function CabBookings() {
  ...
  const [bookings, setBookings] = useState<CabBooking[] | null>(null);

  async function handleLookup(e: React.FormEvent) {
    e.preventDefault();
    setBookings(await getCabBookingsByEmail(lookupEmail));
  }

  async function handleCancel(booking: CabBooking) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      <div className="mb-2 flex items-center gap-2 text-ink-900/60">
        ...
        <p className="text-xs font-semibold uppercase tracking-wide">Cab bookings</p>
      </div>
      <h1 className="mb-1 text-2xl font-bold text-ink-950">Manage your rides</h1>
            ...
            <div key={booking.id} className="flex items-center justify-between gap-3 rounded-lg border border-ink-900/10 p-4">
              <div>
                <p className="font-semibold text-ink-950">
                  {booking.pickupLabel.split(",")[0]} &rarr; {booking.dropoffLabel.split(",")[0]}
                ...
                <p className="text-xs text-ink-900/60">
                  {new Date(booking.pickupTime).toLocaleString("en-US", {
                    month: "short",
                    day: "numeric",
                    hour: "numeric",
                    minute: "2-digit",
                  })}{" "}
                  &middot; {formatMoney(booking.fare, booking.currency)}
                </p>
              </div>
