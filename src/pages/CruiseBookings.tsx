...
import { cancelCruiseBooking, getCruiseBookingsByEmail } from "../lib/api";
import type { CruiseBooking } from "../lib/types";
import { formatMoney, formatShortDate } from "../lib/format";

export default function CruiseBookings() {
  const [searchParams] = useSearchParams();
  const [lookupEmail, setLookupEmail] = useState(searchParams.get("email") ?? "");
  const [bookings, setBookings] = useState<CruiseBooking[] | null>(null);

  async function handleLookup(e: React.FormEvent) {
    e.preventDefault();
    setBookings(await getCruiseBookingsByEmail(lookupEmail));
  }

  async function handleCancel(booking: CruiseBooking) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      <div className="mb-2 flex items-center gap-2 text-ink-900/60">
        ...
        <p className="text-xs font-semibold uppercase tracking-wide">Cruise bookings</p>
      </div>
      <h1 className="mb-1 text-2xl font-bold text-ink-950">Manage your cruise bookings</h1>
      ...
      <div className="rounded-xl border border-ink-900/10 bg-white p-6">
        ...
        {bookings !== null && bookings.length === 0 && <p className="text-sm text-ink-900/60">No bookings found for that email.</p>}
            ...
            <div key={booking.id} className="flex items-center justify-between gap-3 rounded-lg border border-ink-900/10 p-4">
              <div>
                ...
                <p className="text-xs text-ink-900/60">
                  {formatShortDate(booking.sailDate)} &middot; {booking.cabinLabel} &middot; {formatMoney(booking.totalUsd, "USD")}{" "}
                  &middot; {booking.bookingReference}
                </p>
              </div>
